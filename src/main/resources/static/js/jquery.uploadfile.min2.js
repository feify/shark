	...
	b.fn.uploadFile = function(f) {
		...
		function r(K, E, x) {
			for (var F = 0; F < x.length; F++) {
				...
				// We only allow one file to upload. So we remove previous statusbar.
				if(E.selectedFiles > 1){
					$("#peng-statusbar").hide().remove();
					K.deleteCallback.call(this, x[F], x[F]);
				}
		...
		function j(y, x) {
			this.statusbar = b("<div id='peng-statusbar' class='ajax-file-upload-statusbar'></div>")
